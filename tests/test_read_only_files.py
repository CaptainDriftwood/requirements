...
def create_requirements_file(base_path: Path, subdir: str, content: str, read_only: bool = False):
...
def test_update_all_files_read_only(runner, tmp_path):
    ...
    file1 = create_requirements_file(tmp_path, "project1", "django==3.0\nrequests==2.25.1\n", read_only=True)
    file2 = create_requirements_file(tmp_path, "project2", "flask==1.1.4\nrequests==2.25.1\n", read_only=True)
...
def test_update_preview_mode_no_warnings(runner, tmp_path):
    ...
    result = runner.invoke(cli, ["update", "requests", "2.30.0", "--preview", str(tmp_path)])
...
def test_sort_all_files_read_only(runner, tmp_path):
    ...
    file1 = create_requirements_file(tmp_path, "project1", "requests==2.25.1\ndjango==3.0\n", read_only=True)
    file2 = create_requirements_file(tmp_path, "project2", "pytest==6.0\nflask==1.1.4\n", read_only=True)
